						...
						<div class="contentArea">
							...
							<div class="btnAndSearch">
								<table class="bas">
									<tr>
										<td id="leftArea">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
											<button class="mdi mdi-delete takeManyBtn" type="button"
												style="color: gray; background: white; border: 0px; font-size: 20px;"></button>
										</td>
											...
											<form id="searchForm" action="search.mil" method="get">
												<div class="searchBar" style="height: 30px;">
													<select name="condition"
														...
														style="height: 30px; width: 250px;"> <input
														type="hidden" name="currentPage" value="1"> <input
														type="hidden" name="mailOwn" value="${ loginUser.empNo }">
													<div
														onclick="document.getElementById('searchForm').submit();"
														class="mdi mdi-magnify"
														style="height: 30px; width: 30px; color: RGB(29, 92, 99); display: inline-block; cursor: pointer;"></div>
												</div>
											</form>
											<!-- 검색바 -->
											<script>
							...
							<table class="boardTable mailTable"
								...
								<thead>
									<tr>
										...
										<th width="80"><label><input type="checkbox"
												class="importbox" id="importAll" name="ichk"><img
												src="resources/images/star_border-black-48dp.svg"></label></th>
								...
								<c:choose>
										...
										<c:forEach items="${ list }" var="b">
											<tr>
												<td><label> <input class="checkbox chk"
														...
														style="vertical-align: middle; transform: scale(1.0);">
									...
									<c:otherwise>
										...
										<c:forEach var="b" begin="1" end="${10-fn:length(list)}">
											<tr>
												<td class="nonContent"><label> </label></td>
							...
							<c:choose>
								<c:when test="${sc eq 1}">
									<!-- 페이징바 -->
									<ul class="pagingBar">
										<li class="pstyle"><a style="width: 50px;" class="pstyle"
											href="search.mil?condition=${condition}&keyword=${keyword}&currentPage=1">first</a></li>
										<c:choose>
											<c:when test="${ pi.currentPage eq 1 }">
												<li><a class="pstyle disabled" href="#"
													style="width: 80px;">previous</a></li>
											</c:when>
											<c:otherwise>
												<li class="pstyle"><a
													href="search.mil?condition=${condition}&keyword=${keyword}&currentPage=${ pi.currentPage-1 }"
													style="width: 80px;">previous</a></li>
											</c:otherwise>
										</c:choose>
										...
										<c:forEach var="p" begin="${ pi.startPage }"
											...
											<c:choose>
												...
												<c:otherwise>
													<li class="pstyle"><a
														href="search.mil?condition=${condition}&keyword=${keyword}&currentPage=${ p }">${ p }</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>
										...
										<c:choose>
											...
											<c:otherwise>
												<li class="pstyle"><a
													href="search.mil?condition=${condition}&keyword=${keyword}&currentPage=${ pi.currentPage+1 }"
													style="width: 50px;">next</a></li>
											</c:otherwise>
										</c:choose>
										<li class="pstyle"><a
											href="search.mil?condition=${condition}&keyword=${keyword}&currentPage=${ pi.maxPage }"
											style="width: 50px;">last</a></li>
									</ul>
									<!-- 페이징바 -->
								</c:when>
								<c:otherwise>
									...
									<ul class="pagingBar">
										<li class="pstyle"><a style="width: 50px;" class="pstyle"
											href="list.mil?currentPage=1&mailOwn=${loginUser.empNo }">first</a></li>
										<c:choose>
											<c:when test="${ pi.currentPage eq 1 }">
												<li><a style="width: 80px;" class="pstyle disabled"
													href="#">previous</a></li>
											</c:when>
											<c:otherwise>
												<li class="pstyle"><a style="width: 80px;"
													href="list.mil?currentPage=${ pi.currentPage-1 }&mailOwn=${loginUser.empNo }">previous</a></li>
											</c:otherwise>
										</c:choose>
										...
										<c:forEach var="p" begin="${ pi.startPage }"
											...
											<c:choose>
												...
												<c:otherwise>
													<li class="pstyle"><a
														href="list.mil?currentPage=${ p }&mailOwn=${loginUser.empNo }">${ p }</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>
										...
										<c:choose>
											...
											<c:otherwise>
												<li class="pstyle"><a style="width: 50px;"
													href="list.mil?currentPage=${ pi.currentPage+1 }&mailOwn=${loginUser.empNo }">next</a></li>
											</c:otherwise>
										</c:choose>
										<li class="pstyle"><a style="width: 50px;"
											href="list.mil?currentPage=${ pi.maxPage }&mailOwn=${loginUser.empNo }">last</a></li>
									</ul>
									<!-- 페이징바 -->
								</c:otherwise>
							</c:choose>
