							...
							<form name="uploadForm" id="uploadForm"
								...
								<table class="mailUpload" id="boardDetail">
									<tr>
										<td colspan="2">
											<div class="left1">
												<c:choose>
													<c:when test="${empty e }">
														<button id="submitBoard" type="button" class="btn1"
															onclick="uploadFile();"
															style="width: 60px; height: 30px; font-size: 0.7em;">
															보내기
													...
													<c:otherwise>
														<button id="submitBoard" type="button" class="btn1"
															onclick="updateLoadFile();"
															style="width: 60px; height: 30px; font-size: 0.7em;">수정</button>
													</c:otherwise>
												</c:choose>
												<button id="submitBoard" type="button" class="btn1"
													onclick="cancleBack();"
													style="width: 60px; height: 30px; font-size: 0.7em;">다시쓰기</button>
									...
									<tr>
										...
										<td>
											<div>
												<input id="n0" type="text" class="inputs" name="mailTo"
													style="width: 950px; float: left;"
													value="${m.mailFrom}@smartdoctor.com">
											</div>
											<div>
												<input id="m1" type="hidden" class="inputs"
													name="mailnameTo">
											</div>
										<td><button type="button" class="middleBtn2"
												onclick="window.open('openSigner.mil','mailAddToList','_blank');">주소록</button></td>
									</tr>
									<tr>
										<th>참조</th>
										<td>
											<div>
												<input id="l0" type="text" class="inputs" name="mailWith"
													style="width: 950px; float: left;" value="${m.mailWith }">
											</div>
											<div>
												<input id="i1" type="hidden" class="inputs"
													name="mailnameWith">
											</div>
									...
									<tr>
										...
										<td colspan="2"><c:choose>
												<c:when test="${f eq 0 }">
													<input type="text" class="inputs" name="mailTitle"
														style="width: 1030px;" value="[RE:]${m.mailTitle }">
												</c:when>
												<c:otherwise>
													<input type="text" class="inputs" name="mailTitle"
														style="width: 1030px;" value="${m.mailTitle }">
												</c:otherwise>
											</c:choose></td>
									</tr>
									<tr>
										<th>첨부파일
											<button type="button"
												class="mdi mdi-arrow-down btn1 fileShow"
									...
									<tr>
										...
										<td colspan="2">
											<div class="fileWrap" style="display: none;">
												...
												<div id="dropZone"
													style="width: 896px; height: 100px; border: 1px solid lightgray;">
													<div id="fileDragDesc">
														<div class="mdi mdi-paperclip"
												...
												<input type="hidden" id="bff" name="boardFileFlag"
													value="${e.mailAttachment }"> <input type="hidden"
													id="fileListSize" name="fileListSize"
													value="${fn:length(mtList)}">
														...
														<tr>
															<c:forEach items="${ mtList }" var="mt">
																<td><input type="hidden" name="fno"
																	value="${mt.attachmentNo}">${mt.attachmentNo}</td>
															</c:forEach>
														</tr>
													</table>
												</div>
									...
									<tr>
										...
										<td colspan="2"><textarea class="textArea" rows="25"
												cols="125" name="mailContent" style="resize: none;">
...
${m.mailContent }
								...
								<input type="hidden" name="mailnameFrom"
									value="${ loginUser.empName }"> <input type="hidden"
									name="empNo" value="${ loginUser.empNo }"> <input
									type="hidden" id="mailNo" name="mailNo" value="${e.mailNo }">
	...
	<script>
		...
		function cancleBack() {
			if ('${param.pageFrom}' == 'inbox') {
				location.href = "list.mil?currentPage=1&mailOwn="
						+ '${loginUser.empNo}';
			} else if ('${param.pageFrom}' == 'detail') {
				location.href = "list.mil?currentPage=1&mailOwn="
						+ '${loginUser.empNo}';
			} else if ('${param.pageFrom}' == 'import') {
				location.href = "ilist.mil?currentPage=1&mailOwn="
						+ '${loginUser.empNo}';
			} else {
				location.href = "flist.mil?currentPage=1&mailOwn="
						+ '${loginUser.empNo}';
			}
